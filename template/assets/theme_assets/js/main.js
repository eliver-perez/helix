...
function manageIconsUni(target, data) {
  ...
  xhr.open('GET', typeof env !== "undefined" && env.iconLoaderUrl ? env.iconLoaderUrl : data, true);
  if (selector) {
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        const colUni = uniIcons.map(icon =>
          `
  ...
  if (selector) {
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        const colLine = LineIcons.map(icon =>
          `
          <div class="col-span-12 2xl:col-span-3 sm:col-span-6 lg:col-span-4">
           <div class="flex items-center gap-[5px]">
...
function switchGroup(selector, target) {
  ...
  if (switchSelector) {
    ...
    function toggleCheckboxes() {
      ...
      var isAnyUnchecked = Array.from(checkboxes).some(checkbox => !checkbox.checked);
...
function setupProductCount(wrapperSelector, totalNumberSelector, downSelector, upSelector, separation) {
  ...
  if (productContainer) {
    ...
    if (decrementBtns.length > 0 && incrementBtns.length > 0) {
      ...
      function startDecrement() {
        ...
        if (row) {
          decrement(row);
          intervalIds.set(row, setInterval(() => decrement(row), 200));
        }
      }
      ...
      function startIncrement() {
        ...
        if (row) {
          increment(row);
          intervalIds.set(row, setInterval(() => increment(row), 200));
        }
      }
  ...
  if (table) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      row.addEventListener('dragend', function (e) {
        ...
        table.querySelectorAll('.border-t-2', '.border-primary/30').forEach(function (el) {
...
function rainColorSwitch(){
  ...
  inputs.forEach(input => {
    ...
    const initialColor = getComputedStyle(document.documentElement).getPropertyValue(`--${input.id}`).trim();
...
function content_ready_scripts() {
  ...
  manageUpcomingEvents('.event-title', '.edit-event-title', '.remove-event-wrapper');
  manageUpcomingEvents('.task-title', '.edit-task-title', '.remove-task-wrapper');
  ...
  placeholderImage('.placeholder-image', 'images/avatars/placeholder-image.png');
  imageUpload('profile-upload-image', 'profile-dropzone-file', 'profileImage');
  imageUpload('widget-profile-upload-image', 'widget-profile-dropzone-file', 'widgetProfileImage');
  socialPost();
  ...
  setupProductCount('.product-container', '.productItemsNumber', '.productItemsDown', '.productItemsUp', '.productItems');
  zoomHover('.zoomable');
